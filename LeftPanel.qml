...
ColumnLayout {
    ...
    // Calculates button height based on the bottom panel height (20% of total)
    // minus margins (30) and spacing (24 for 2 gaps), divided by 3 rows.
    property int dynamicRowHeight: Math.max(30, (bottomControls.height - 60) / 3)

    // Keep original property names but bind them to the dynamic calculation
    property int row1Height: dynamicRowHeight
    property int row2Height: dynamicRowHeight
    property int row3Height: dynamicRowHeight

    // Property bound from Main.qml
    property string currentCoordinateSystem: "Cartesian"

    // --- 1. Header Card (5% Height) ---
    PanelCard {
        id: headerCard
        Layout.fillWidth: true

        // Layout Weight Strategy: 5
    ...
    // --- 3. Bottom Operation Controls (30% Height) ---
    PanelCard {
        id: bottomControls
        ...
        GridLayout {
            ...
            // Row 1
            VectorButton { id: onOffBtn; Layout.fillWidth: true; Layout.fillHeight: true; property bool isOn: false; label: isOn ? "On" : "Off"; pathData: isOn ? icons.toggleOn : icons.toggleOff; baseColor: isOn ? "#76ff03" : "#546E7A"; onClicked: isOn = !isOn }
            VectorButton { Layout.fillWidth: true; Layout.fillHeight: true; pathData: icons.home; baseColor: "#0091EA"; label: "Home" }
            VectorButton { id: runBtn; Layout.fillWidth: true; Layout.fillHeight: true; property bool isRunning: false; label: isRunning ? "Pause" : "Run"; pathData: isRunning ? icons.pause : icons.play; baseColor: isRunning ? "#FF9800" : "#00b341"; onClicked: isRunning = !isRunning }
            VectorButton { id: startStopBtn; Layout.fillWidth: true; Layout.fillHeight: true; property bool isStarted: false; label: isStarted ? "Stop" : "Start"; pathData: icons.power; baseColor: isStarted ? "#D50000" : "#455A64"; onClicked: isStarted = !isStarted }
            VectorButton { Layout.fillWidth: true; Layout.fillHeight: true; pathData: icons.exit; baseColor: "#FF3D00"; label: "Exit" }
            VectorButton {
                id: modeBtn; Layout.fillWidth: true; Layout.fillHeight: true; property string currentMode: "Sim"; label: "M: " + currentMode; pathData: currentMode === "Sim" ? icons.simEye : icons.robot; baseColor: currentMode === "Sim" ? "#2E7D32" : "#607D8B"; onClicked: selectionPopup.open()
                Popup {
                    id: selectionPopup
            ...
            VectorButton {
                ...
                Popup {
                    ...
                    y: -height - 10
                    width: 220
                    height: 340 // Increased height to accommodate the new button
                    padding: 10
                    closePolicy: Popup.CloseOnEscape | Popup.CloseOnPressOutsideParent
                    ...
                    contentItem: Item {
                        // --- VIEW 1: SELECT FILE TYPE ---
                        ColumnLayout {
                            anchors.fill: parent
                            visible: !filesBtn.showingActions
                            spacing: 5
                            Text {
                                Layout.alignment: Qt.AlignHCenter
                                text: "SELECT TYPE"
                            ...
                            ScrollView {
                                ...
                                ColumnLayout {
                                    ...
                                    VectorButton {
                                        ...
                                        onClicked: {
                                            filesBtn.selectedCategory = "PR"
                                            filesBtn.activeIndex = 0
                                            filesBtn.showingActions = true
                                        }
                                    }
                                    VectorButton {
                                        label: "TR Files"
                                        ...
                                        Layout.fillHeight: false
                                        onClicked: {
                                            filesBtn.selectedCategory = "TR"
                                            filesBtn.activeIndex = 1
                                            filesBtn.showingActions = true
                                        }
                                    }
                                    VectorButton {
                                        ...
                                        Layout.fillHeight: false
                                        onClicked: {
                                            filesBtn.selectedCategory = "TP"
                                            filesBtn.activeIndex = 2
                                            filesBtn.showingActions = true
                                        }
                                    }
                        ...
                        // --- VIEW 2: ACTIONS FOR SELECTED TYPE ---
                        ColumnLayout {
                            anchors.fill: parent
                            visible: filesBtn.showingActions
                            spacing: 10
                            RowLayout {
                                Text {
                                    text: filesBtn.selectedCategory + " ACTIONS"
                                    color: "white"
                                    font.bold: true
                                    font.pixelSize: 12
                                }
                                ...
                                Button {
                                    text: "Back"
                                    flat: true
                                    onClicked: filesBtn.showingActions = false
                                    contentItem: Text {
                                        text: parent.text
                                        color: "white"
                                        font.bold: true
                            ...
                            VectorButton {
                                label: "New " + filesBtn.selectedCategory
                                baseColor: "#0091EA"
                                Layout.fillWidth: true
                                Layout.preferredHeight: 50
                                Layout.fillHeight: false
                                onClicked: {
                                    filesBtn.lastAction = "New Created"
                                    filesPopup.close()
                                }
                            }

                            VectorButton {
                                label: "Open " + filesBtn.selectedCategory
                                baseColor: "#0091EA"
                                Layout.fillWidth: true
                                Layout.preferredHeight: 50
                                Layout.fillHeight: false
                                onClicked: {
                                    filesBtn.lastAction = "Opened"
                                    filesPopup.close()
                                }
                            }
                            ...
                            // --- NEW DELETE BUTTON ---
                            VectorButton {
                                label: "Delete " + filesBtn.selectedCategory
                                baseColor: "#D32F2F" // Danger/Red color
                                Layout.fillWidth: true
                                Layout.preferredHeight: 50
                                Layout.fillHeight: false
                                onClicked: {
                                    filesBtn.lastAction = "Deleted"
                                    filesPopup.close()
                                }
                            }
